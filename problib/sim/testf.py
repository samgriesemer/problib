...
class Env():
    ...
    environments to be used inside a Gym or interacting with Agent types should inherit
    this interface. Need to provide a state and action space for defining valid actions
    and return states.
    ...
    def draw(self):
        """
        Return view of internal environment state.

        This method is currently undefined as there is no standard view protocol. This may
        ultimately be removed considering visualization is left to client side Javascript
    ...
    def create(self, type, params):
        """
        Create an entity of the specified type, use the given params. Used to populate the
        environment with objects necessary to the simulation. This method is provided for
        working with environments that need to be dynamic and update the entities within
...
class Static(Env):
    """
    Environment for defining a static state.

    To be used as a simple environment object inheriting necessary interface when some
    static context is needed for agent reference. Adds nothing to the base class.
    """
    pass

class RandomState(Env):
    """
    Simple random state environment.

    Takes given state, action, and entity spaces and simply samples random states at each
    tick. This env independent of any assigned agents.
    """
...
class Grid(Env):
    """
    Naive implementation, simple velocity and position updates on body of defined point
    objects. Action space includes all real number within velocity ranges, and the state
    is the current set of physics entities and their 2D positions.

    For now, the state (method) property is not resitricted and is encouraged to update
    state after entity creation/removal has occurred. No other entity state's are
    affected, so no standard implementation should expect any side effects. Ultimately
    this may need reconsideration, but it has backing as explained in the docs.
    """
    ...
    def tick(self, actions={}):
        """
        Handles action requests to engine entities, returns resulting state. Note for
        multi-agent systems, `action` is a list of actions to be performed by each agent
        at the current step.
